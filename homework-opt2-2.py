...
def variational_solver_2d(
    ...
    for i in range(max_iterations):
        ...
        print(f"近似解 (x,y): ({params[0]:.8f}, {params[1]:.8f})")
        print(f"f(x,y) = {current_loss:.8f}")
...
# 定义目标函数（计算到所有点的距离总和）
def f(point: jnp.ndarray) -> float:
    total_distance = 0.0
    for i in range(0, len(poi), 2):
        dx = point[0] - poi[i]
        dy = point[1] - poi[i+1]
        total_distance += jnp.sqrt(dx**2 + dy**2)
    return total_distance

# 求解
initial_guess = list(map(float, input("请输入初始猜测值(x,y): ").split(',')))
...
print(f"\n最优解: (x, y) ≈ ({solution[0]:.8f}, {solution[1]:.8f})")
